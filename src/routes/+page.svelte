<script lang="ts">
  ...
  import NoBookSelected from '$lib/components/NoBookSelected.svelte';
  import BookOpenError from '$lib/components/BookOpenError.svelte';
  import FloatingToolbar from '$lib/components/FloatingToolbar.svelte';
  import RememberLastPage from '$lib/components/RememberLastPage.svelte';
  import AnnotationPersistence from '$lib/components/AnnotationPersistence.svelte';
  ...
  // NOT: Mercek/Editör/Video-Ses etkileşim katmanı (ZBookInteractionLayer,
  // BookInteractionsPanel) kasıtlı olarak devre dışı bırakıldı — ilk sürüm
  // sade tutuluyor. Kod silinmedi, src/lib/zbook/ altında duruyor.
  ...
  // Açılacak kitap: ?url=<pdf adresi>&id=<benzersiz kitap kimliği>&title=<başlık>
  // id verilmezse url kendisi kimlik olarak kullanılır (gizli cevaplar bu
  // kimliğe göre saklanır).
  const bookUrl = $derived(page.url.searchParams.get('url') ?? '');
  const bookId = $derived(page.url.searchParams.get('id') || bookUrl);
  const bookTitle = $derived(page.url.searchParams.get('title') ?? 'Ders Kitabı');
  ...
  $effect(() => {
    if (bookId) teacherStore.loadForBook(bookId);
  });
  ...
  let openTimedOut = $state(false);
  let openError = $state('');

  async function handleInitialized(registry: PluginRegistry) {
    if (!bookUrl) return; // Kitap belirtilmemiş — NoBookSelected ekranı gösterilecek.
    openTimedOut = false;
    openError = '';
    ...
    const task = documentManager?.openDocumentUrl({
      url: bookUrl,
      documentId: bookId,
      // 'auto': sunucunun Range isteği desteğini dener, desteklemiyorsa
      // tüm dosyayı indirmeye otomatik geri döner.
      mode: 'auto',
    });
    task?.wait(
      () => {
...
<svelte:head>
  <title>{bookTitle} — Kitabı Aç</title>
</svelte:head>
...
{#if pdfEngine.error}
  <div class="flex h-screen items-center justify-center">
    <div class="rounded-lg bg-red-50 p-6 text-red-900">
      <h2 class="mb-2 text-lg font-semibold">Motor Hatası</h2>
          ...
          {#if pluginsReady}
            <div class="flex h-full flex-col">
              {#if !bookUrl}
                <NoBookSelected />
              {:else if openError}
                <BookOpenError message={openError} onRetry={retryOpen} />
              ...
              {:else if openTimedOut}
                <BookOpenError
                  message="Kitap 20 saniyede açılamadı. Bağlantı ya da dosya adresi hatalı olabilir."
                  onRetry={retryOpen}
                />
              {:else}
                <div class="flex h-full items-center justify-center">
                  <LoadingSpinner message="Kitap açılıyor..." />
                </div>
              {/if}
            </div>
          {:else}
            <div class="flex h-full items-center justify-center">
              <LoadingSpinner message="Hazırlanıyor..." />
            </div>
          {/if}
        {/snippet}
      </EmbedPDF>
    ...
    <div class="flex-1 overflow-hidden">
      <DocumentContent {documentId}>
        {#snippet children({ documentState, isLoading, isError, isLoaded })}
          {#if isLoading}
            <div class="flex h-full items-center justify-center">
              <LoadingSpinner message="Kitap yükleniyor..." />
            </div>
          {/if}
          {#if isError}
            {#if documentState?.errorCode === 4}
              <DocumentPasswordPrompt {documentState} />
            {:else}
              <BookOpenError
                message={documentState?.error ?? 'Kitap açılamadı.'}
          ...
          {/if}
          {#if isLoaded}
            <FloatingToolbar {documentId} />
            <RememberLastPage {documentId} />
            <AnnotationPersistence {documentId} />
            <TeacherControls {documentId} />
            <CurtainOverlay />
            <div
              class="relative h-full w-full"
              style={toolbarState.docked
                ? `padding-${toolbarState.side}:44px;`
                : ''}
            >
